...
#[test]
fn v4_migration_is_a_no_op_instead_of_a_formatter() {
    let legacy = TempConfig::new(&v3_config());
    let migrated = plan_migration(&legacy.path, Some(&legacy.path), 4)
        .unwrap()
        .target_toml;
    let current = TempConfig::new(&migrated);
    let plan = plan_migration(&current.path, Some(&current.path), 4).unwrap();
    assert!(!plan.changed);
    assert_eq!(plan.target_toml, migrated);
}
