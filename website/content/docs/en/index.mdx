...
2. Integrate sensor channels.
3. Fit the recorded function-generator sine wave sent to the Sagnac EOM and
   prepare the shared analysis window/output grid once.
4. Average the configured raw `[[signals]]` channels over the lock-in window and
   save the combined `signal/mean.png` figure.
5. Demodulate signals with the configured `boxcar_legacy` lock-in filter.
6. Align the phase and extract the Kerr angle.
