...
description: エクスポートされた機械向けresourceの形式、安定性境界、integrity確認。
---
...
各page entryにはlocale、title、description、canonical HTML URL、Markdown URL、
エクスポート済みMarkdown byte列のSHA-256 digestを含む。manifestにはtop-level machine endpointとmedia typeも含む。
...
同一source commitからdeterministic feedを生成。consumerはmanifest schema 1に追加されたunknown fieldを無視可能。
breakingなshape変更にはschema更新が必要。
