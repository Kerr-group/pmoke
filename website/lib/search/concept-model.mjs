...
const CONCEPTS = [
  ...
  ['quickstart', ['quickstart', 'quick start', 'first run', 'complete analysis run', 'chain', 'クイックスタート', '最初の解析', '一括実行']],
  ['install', ['install', 'installation', 'setup', 'build from source', 'source build', 'python', 'prerequisite', 'analysis-only installation', '導入', 'インストール', 'セットアップ', 'ビルド', '前提環境', '解析専用 build']],
  ['feature', ['feature flag', 'analysis only', 'analysis-only', 'build features', 'cargo feature', 'hardware feature', 'feature selection', 'visa compile', '機能フラグ', 'feature flag', 'cargo feature', 'feature の選択', 'visa compile']],
  ['config', ['config', 'configuration', 'toml', 'setting', '設定', '構成', 'toml']],
  ['validate', ['validate', 'validation', 'diagnostic', 'invalid', 'migrate', 'migrate legacy', 'legacy configuration', 'configuration error', 'error path', 'field path', '不正な toml', '検証', '診断', '設定ミス', '不正', 'legacy 設定の移行', 'field path']],
  ['schema', ['schema', 'json schema', 'field reference', 'parameter reference', 'スキーマ', 'schema', 'フィールド', 'パラメーター']],
  ['connect', ['connect', 'connection', 'transport', 'timeout', '通信', '接続', 'トランスポート']],
  ...
  ['kerr', ['kerr', 'magneto-optic', 'moke', 'rotation angle', 'カー角度', 'カー角', '磁気光学', 'moke']],
  ['harmonics', ['harmonic', 'harmonics', 'six harmonic', '高調波', 'ハーモニクス', '六次高調波']],
  ['analysis', ['analyze', 'calculate', 'analysis pipeline', 'processing', '解析', '解析パイプライン', '処理']],
