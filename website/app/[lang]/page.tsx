...
const copy = {
  en: {
    ...
    lead: 'Reproducible pulsed-field MOKE measurements',
    description:
    ...
    signal: {
      ...
      description:
        'Pulsed-field MOKE measurement workflow. A unipolar magnetic-field pulse defines the common time axis for lock-in X/Y extraction, phase correction, and Kerr-angle calculation.',
      ...
      fieldSummary: 'The unipolar magnetic-field pulse starts at the 0 ms trigger.',
      lockInSummary: 'Extract lock-in X/Y on the same time axis as the magnetic-field pulse.',
      phaseSummary: 'Rotate the lock-in X/Y vector to bring the quadrature component to zero.',
      kerrSummary: 'Calculate the Kerr angle from the phase-corrected signal and display it in mrad.',
      currentTime: 'TIME',
      timeAxis: 't (ms)',
      fieldAxis: 'μ₀H (T)',
      ...
      liY: 'LI Y',
      rawVector: 'RAW X/Y',
      correctedVector: 'CORRECTED X/Y',
      phaseShift: 'PHASE ROTATION',
      quadratureZero: 'CORRECTED Y′',
    ...
    cards: [
      ...
      ['Analysis workflow', 'Extract lock-in X/Y, correct the phase, and calculate the Kerr angle.', 'activity'],
      ['Documentation search', 'Search the English and Japanese docs and access text exports for AI tools.', 'search'],
    ],
  },
  ja: {
    kicker: 'PULSED-FIELD MOKE MEASUREMENTS',
    lead: 'パルス磁場下での再現可能なMOKE測定',
    description:
    ...
    signal: {
      ...
      description:
        'パルス磁場下でのMOKE測定を示す処理図。単極性磁場パルスと共通の時間軸上でロックインX/Yを抽出し、位相補正後にKerr角度を算出。',
      pipeline: '測定ワークフロー',
      sequence: 'パルス磁場下でのMOKE測定と解析の工程',
      fieldPulse: '磁場パルス',
      lockIn: 'ロックインX/Y',
      phaseCorrection: '位相補正',
      kerrAngle: 'Kerr角度',
      fieldSummary: '0 msのトリガーで、単極性磁場パルスが立ち上がる。',
      lockInSummary: '磁場パルスと共通の時間軸上で、ロックインX/Yを抽出。',
      phaseSummary: 'ロックインX/Yベクトルを回転し、直交成分がゼロになるよう位相を補正。',
      kerrSummary: '位相補正後の信号からKerr角度を算出し、mrad単位で表示。',
      currentTime: '時刻',
      timeAxis: 't (ms)',
      fieldAxis: 'μ₀H (T)',
      ...
      liY: 'LI Y',
      rawVector: '補正前 X/Y',
      correctedVector: '補正後 X/Y',
      phaseShift: '位相回転量',
      quadratureZero: '補正後 Y′',
      kerrResult: 'Kerr角度',
      replayStage: 'この工程を再生',
    },
    ...
    cards: [
      ...
      ['解析ワークフロー', 'ロックインX/Yの抽出、位相補正、Kerr角度の算出を一つの流れで実行。', 'activity'],
